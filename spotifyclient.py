...
class SpotifyClient:
    ...
    def __init__(self, client_id, user_id, client_secret, redirect_uri):
        ...
        self.spotipyObject = spotipy.Spotify(auth_manager=SpotifyOAuth(scope="user-library-read app-remote-control user-read-playback-state user-read-currently-playing user-modify-playback-state user-read-recently-played playlist-modify-public", client_id=client_id, client_secret=client_secret, redirect_uri=redirect_uri))


    def get_recent_tracks(self, limit=10):

        res = self.spotipyObject.current_user_recently_played(limit=limit) 
        tracks = [Track(track["track"]["name"], track["track"]["uri"], track["track"]["artists"][0]["name"]) for 
                    track in res["items"]]
        return tracks
    ...
    def get_recommendations(self, seed_tracks, limit):
        ...
        response = self.spotipyObject.recommendations(seed_tracks=seed_tracks_uri, limit=limit)
        tracks = [Track(track["name"], track["uri"], track["artists"][0]["name"]) for
                  track in response["tracks"]]
        return tracks
    ...
    def fill_playlist(self, tracks, playlist):
        ...
        self.spotipyObject.user_playlist_add_tracks(user="apjohnson12", playlist_id=playlist.id, tracks=tracks_uri)
    ...
    def get_categories(self):
        ...
        categories = [Category(category["name"], category["id"]) for category in categories]   

        return categories
    
    def get_category_playlists(self, category_id):

        results = self.spotipyObject.category_playlists(category_id, limit="5", offset=str(random.randint(1,4)))["playlists"]["items"]
    ...
    def grab_playlist_songs(self, playlist_id):
        ...
        for mega_track in results["tracks"]["items"]:
            try:
                all_tracks.append(Track(mega_track["track"]["name"], mega_track["track"]["uri"], mega_track["track"]["artists"][0]["name"]))
            except Exception:
                pass
    ...
    def get_current_playback(self):
        ...
        return self.spotipyObject.current_user_playing_track()
